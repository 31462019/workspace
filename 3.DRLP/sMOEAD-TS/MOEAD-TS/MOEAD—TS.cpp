...
void CMOEAD::TabuSearch()
{
	...
	for(int i = 0; i < NumOfSub/10; i ++)
	{
		...
		double fit = CurrentPopulation[iPop].CurrentSolution.ComputingFitnessValue(CurrentPopulation[iPop].WeightVector.lambda, strFitnessType);
		...
		while(count < 100){
			...
			sub.SinglePointInsert(Current, CurrentPopulation[iPop].WeightVector.lambda, strFitnessType);			
			/*
			...
			*/
			sub.SinglePointXover(mark, Current, CurrentPopulation[iPop].WeightVector.lambda, strFitnessType);
			//---------------------------
