...
void CIndividual::Randomize()
{
	...
	//printf("Umax: %f \n",Umax);	
	for(int i =0;i<BreakNum[1]+BreakNum[2];i++){
		Umax -= WidOfRoom[Arrange[i+BreakNum[0]]];
		//printf("Umax: %f \n",Umax);
	}
	//printf("Umax Done!!!: %f \n",Umax);
	if(Umax <LimOfSecRow[0] ||Umax<LimOfSecRow[1]){
		Gap[0]<LimOfSecRow[0];
		Gap[1]<LimOfSecRow[1];
		return;
	}
	Gap[0] = LimOfSecRow[0] +Rnd.GetNumber()*(Umax - LimOfSecRow[0]);
	Gap[1] = LimOfSecRow[1] +Rnd.GetNumber()*(Umax - LimOfSecRow[1]); 
...
bool CIndividual::IsFeasible()
{
	...
	if(LimOfRow[1]-sum -Gap[0]-Gap[1] < LimOfSecRow[2])
		return false;
...
void CIndividual::SinglePointInsert(CIndividual &parent, vector<double> &lambda, char *strFuncType)		//insert
{
	vector<double> sub_obj = parent.ObjValue;
	double fitness = parent.ComputingFitnessValue(lambda,strFuncType);
	//printf("fitness %f \n",fitness);
	...
	if( BreakNum[0] > 1){
		for (int i = 0; i < parent.BreakNum[0]; ++i){
			for (int j = 0; j < parent.BreakNum[3] + 1; ++j){
				...
				if (this->ComputingFitnessValue(lambda,strFuncType) < fitness)
				{
					fitness = this->ComputingFitnessValue(lambda,strFuncType);
					s_a = i;s_b = j;
					tag = 1;
				}
...
void CIndividual::SinglePointXover(vector<int> &mark ,CIndividual &parent, vector<double> &lambda, char *strFuncType)
{
